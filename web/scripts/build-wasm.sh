...
# they downloaded first, and new page code then called into old glue: an iPhone reported
# "prove_h_only is not a function" against a deployment where that export plainly existed.
# Revalidating instead would fix the next visitor and not this one, since `immutable` means
# a reload does not even ask. A changed URL fixes both, and lets the long cache stay honest.
#
# Stamped into the bundle rather than fetched at runtime, so it costs no request and cannot
# itself be the stale thing. This file is generated, gitignored, and written by this script
# only: if it is missing, `vite build` fails loudly, which is the right answer to building
# the page without building the prover it loads.
hash="$(shasum -a 256 "$here/../static/pkg/g16_wasm_bg.wasm" | cut -c1-12)"
cat > "$here/../src/lib/pkg-version.ts" <<TS
/// Generated by scripts/build-wasm.sh. Do not edit, do not commit.
///
/// The first twelve hex of the built wasm's sha256, appended to both static/pkg URLs so a
/// new prover is a new URL. See the note in that script for what went wrong without it.
export const PKG_VERSION = '$hash';
TS
printf 'pkg version           %10s\n' "$hash"
