...
//
// 2. ONLY ONE WINDOW WIDTH IS COMPILED. Metal compiles four widths per group in 54 ms
//    of runtime MSL; NVRTC costs 113.6 s of source-to-PTX plus about 175 s of driver
//    JIT for the MSM unit alone on a cold T4 (g16-cuda/src/context.rs), and
...
//    through a whole power-16 prepare, fft.rs `FFT_WINDOW`) is instantiated. Sweeping
//    `c` on NVIDIA means adding `FFT_KERNELS` lines at the bottom and widths to
//    `FFT_WINDOWS` in g16-cuda/src/fft.rs, and paying the compile once.
...
template <typename F, u32 C>
__device__ __forceinline__ void fft_mix_impl(const Xyzz<F>* in,
                                             Xyzz<F>* out,
                                             const u32* tw,
                                             FftParams p,
                                             u32 tid) {
...
// ladders.
template <typename F, u32 C>
__device__ __forceinline__ void fft_mix_scale_impl(const Xyzz<F>* in,
                                                   Xyzz<F>* out,
                                                   const u32* stw,
                                                   FftParams p,
                                                   u32 tid) {
...
// function by name. g16-cuda/src/fft.rs builds the same names from the same width list.
// ---------------------------------------------------------------------------

#define FFT_KERNELS(SUF, FT, PTT, C)                                          \
    extern "C" __global__ void fft_mix_##SUF(const PTT* in,                   \
                                             PTT* out,                        \
                                             const u32* tw,                   \
                                             FftParams p) {                   \
    ...
    extern "C" __global__ void fft_mix_scale_##SUF(const PTT* in,             \
                                                   PTT* out,                  \
                                                   const u32* stw,            \
                                                   FftParams p) {             \
        u32 tid = blockIdx.x * blockDim.x + threadIdx.x;                      \
