//! The packed layouts that cross the Rust/MSL boundary, defined once.
//!
//! # This file and `src/shaders/*.metal` must be changed together
//!
//! Every `#[repr(C)]` type below has a byte-for-byte twin declared in
//! [`crate::kernels::FR_MSL`] (`src/shaders/bn254_fr.metal`). The MSL side carries a
//! `static_assert` on each `sizeof`, this side carries a `const` assertion on each
//! `size_of`, and [`tests::msl_declares_the_same_constants`] greps the shader source for
//! the exact constant lines so the two cannot drift silently. If you change a struct here
//! you are changing a wire format; go change the shader in the same commit.
//!
...
//! `Vec<G1Affine>` into a Metal buffer hands the GPU a 72-byte stride while the shader
...
//! exactly the radix the GPU CIOS routine uses. So:
//!
...
pub(crate) mod tests {
    ...
    /// Deterministic, dependency-free, and good enough for "did the limbs survive the
    /// trip". SplitMix64, Steele et al. A real RNG would mean adding a dev-dependency to
    /// a crate whose whole point is to have few of them.
    ...
    impl SplitMix64 {
        ...
        pub(crate) fn next_fr(&mut self) -> Fr {
            let mut b = [0u8; 32];
