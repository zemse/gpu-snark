...
/// The circuit, the five MSM jobs and the `g16_core::Backend` implementation. The async
/// halves compile everywhere; the blocking `PreparedCircuit` impl over them is native only.
pub mod backend;
pub mod batch;
pub mod device;
pub mod gather;
