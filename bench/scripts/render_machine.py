...
def main():
    ...
    ap.add_argument("--gpu", default="",
                    help="accelerator name, e.g. 'Tesla T4'. Recorded as its own field "
                         "rather than only smuggled into the machine slug, so a reader "
                         "and the README generator can both find it.")
    ap.add_argument("--cpu", default="",
                    help="host CPU model string, for the same reason")
    ...
    if meta:
        L.append(f"`{meta.get('arch','?')}`, {meta.get('cores','?')} logical cores, "
                 f"os `{meta.get('os','?')}`")
        # Hardware as named fields. The machine slug encodes some of this already, but a
        # slug is an identifier and not a description: "aws-g4dn.2xlarge-tesla-t4" is fine
        # for a filename and poor as a heading, and nothing downstream could recover the
        # accelerator from it without guessing where the name ended.
        if a.cpu:
            L.append(f"cpu: {a.cpu}")
        if a.gpu:
            L.append(f"accelerator: {a.gpu}")
        L.append("")
